...
# all messages that are not our test messages will be sent at mindelay
# (0.5 second). re-sends of our test messages will be sent at maxdelay
# (10 seconds)

# starting time 30:
#
# p1 sends, only p2 gets msg (p3 & p4 copies are dropped).
#
# then p2 sends and immediately crashes, p1 & p3 get msg.  -> p3 knows
# it misses p1's first msg -> so p3 must re-request from p1, because
# p2 has crashed
#
# p3 sends then immediately crashes, p4 receives -> p4 knows it missed
# p1 and p2's first messages -> must re-request from p1 (because p3
# has crashed) once p1 sends a 2nd msg and p4 receives it
...
<p1> T3xM3u6w_b:   this is 4th from p1 _4112_   W91XeGg9 T3xM3u6w_e
<p2> T3xM3u6w_b:   2nd from p2 _4212_   Br/r75bn T3xM3u6w_e
<p3> T3xM3u6w_b:   2nd msg from p3 _3122_   yNFB8aD2 T3xM3u6w_e
<p1> T3xM3u6w_b:   this is 5th from p1 _5222_   ogP4XwmN T3xM3u6w_e
'''.splitlines()
    ,
...
wait 1
#
# tx at 31, rx at (-, 32, x, x)
send T3xM3u6w_b:   this is 4th from p1 _4112_   W91XeGg9 T3xM3u6w_e
#
wait 20
#
# tx at 51, rx at (-, 52, ., .)
send T3xM3u6w_b:   this is 5th from p1 _5222_   ogP4XwmN T3xM3u6w_e
#
wait 30
...
# tx at 33, rx at (34, -, 34, x)
send T3xM3u6w_b:   2nd from p2 _4212_   Br/r75bn T3xM3u6w_e
#
# need to wait past 34 seconds so the above message can be sent (since
# we are simulating delay at the sender) out, but crash before
# receiving any requests/test messages from others
wait 1.3
#
# at 34.3
crash .
...
wait 7
# have received p2 _4212_: it cant deliver because it has not
# delivered p1 _4112_ msg -> thus its current vector timestamp is
# still _3112_
#
# tx at 37, rx at (38, ., -, 38)
send T3xM3u6w_b:   2nd msg from p3 _3122_   yNFB8aD2 T3xM3u6w_e
#
wait 1.3
#
# at 38.3
...
delaydropspecs = {
    ...
    '|'.join(['T3xM3u6w_b:   this is 4th from p1 _4112_   W91XeGg9 T3xM3u6w_e',
              '0', '1000000', '-1', '-1']) + '\n'
    +
    '|'.join(['T3xM3u6w_b:   this is 5th from p1 _5222_   ogP4XwmN T3xM3u6w_e',
    ...
    +
    '|'.join(['T3xM3u6w_b:   2nd from p2 _4212_   Br/r75bn T3xM3u6w_e',
              '1000000', '0', '1000000', '-1']) + '\n'
    ,
    ...
    +
    '|'.join(['T3xM3u6w_b:   2nd msg from p3 _3122_   yNFB8aD2 T3xM3u6w_e',
