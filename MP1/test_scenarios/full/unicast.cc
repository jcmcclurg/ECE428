...
static int attemptsendcounts[MAXNUMMEMBERS] = {0};
...
void usend(int dest, const char *message, int len) {
    ...
    /* simple count of attempt to send to this dest */
...
//            debugprintf("dest idx: %d, delay: %f\n", memberidx, msgdelay->delays[memberidx]);
    ...
    if (msgidx == g_nummsgs) {
        ...
        if (mystrstr(message, len, TESTMSG_BEGIN_MARKER) != NULL) {
            ...
            /* this is trying to resend someone else's message --> use
             * maxdelay
             */
            delay = MAXDELAY;
